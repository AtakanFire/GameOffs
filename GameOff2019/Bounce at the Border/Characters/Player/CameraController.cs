...
public class CameraController : Spatial
{
    ...
    public override void _UnhandledInput(InputEvent @event){
        ...
        if (((Input.IsActionPressed("RightMouseButton") || Input.IsActionPressed("LeftMouseButton")) && mouseMovement.Length() >= 1)
            || (Input.IsActionPressed("RightMouseButton") && Input.IsActionPressed("LeftMouseButton")))
        {
            Input.SetMouseMode(Input.MouseMode.Captured);
            autoLookForward = false;
        }
        else if (Input.IsActionJustReleased("RightMouseButton") || Input.IsActionJustReleased("LeftMouseButton"))
        {
            Input.SetMouseMode(Input.MouseMode.Visible);
        }

        if (Input.IsActionJustPressed("Forward") && !(Input.IsActionJustReleased("RightMouseButton") || Input.IsActionJustReleased("LeftMouseButton")))
        {
    ...
    private void CameraRotation(float delta)
    {
        ...
        if ((Input.IsActionJustPressed("RightMouseButton") && Input.IsActionJustPressed("LeftMouseButton")) ||
            (Input.IsActionJustPressed("RightMouseButton") && !Input.IsActionJustPressed("LeftMouseButton")))
        {
            lookForward = true;
        } 
        else if((Input.IsActionPressed("RightMouseButton") && Input.IsActionPressed("LeftMouseButton")) ||
            (Input.IsActionPressed("RightMouseButton") && !Input.IsActionPressed("LeftMouseButton"))) 
        {
            if (!lookForward  && Input.GetMouseMode() == Input.MouseMode.Captured)
            {
        ...
        else if (Input.IsActionPressed("LeftMouseButton") && !Input.IsActionPressed("RightMouseButton"))
        {
            lookForward = false;
